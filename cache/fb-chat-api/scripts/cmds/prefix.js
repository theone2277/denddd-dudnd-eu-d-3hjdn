...
module.exports = {
	...
	langs: {
		...
		en: {
			...
			myPrefix: "Hello! Me is neko!\nHere's my prefix:\n😻Neko prefix system: %1\n😼Neko prefix in your box: %2"
		}
	},
...
onChat: async function ({ event, message, getLang }) {
  if (event.body) {
    ...
    const prefixesToCheck = ["neko", "bot", "prefix"];
    ...
    if (prefixesToCheck.includes(lowercasedMessage.trim())) {
      // Check if the message contains only the specified prefix
      if (event.body.trim().toLowerCase() === lowercasedMessage) {
        return () => {
