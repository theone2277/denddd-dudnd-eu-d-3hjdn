  ...
  onStart: async function ({ args, threadsData, message, event, usersData }) {
    if (args[0] == "group" || args[0] == "thread") {
      ...
      try {
        ...
        return message.reply(targetID == event.threadID ? "✅ | Đã làm mới thông tin nhóm chat của bạn thành công!" : `✅ | Đã làm mới thông tin nhóm chat ${targetID} thành công!`);
      } catch (error) {
        return message.reply(targetID == event.threadID ? "❌ | Đã xảy ra lỗi không thể làm mới thông tin nhóm chat của bạn" : `❌ | Đã xảy ra lỗi không thể làm mới thông tin nhóm chat ${targetID}`);
      }
    } else if (args[0] == "user") {
      ...
      try {
        ...
        return message.reply(targetID == event.senderID ? "✅ | Đã làm mới thông tin người dùng của bạn thành công!" : `✅ | Đã làm mới thông tin người dùng ${targetID} thành công!`);
      } catch (error) {
        return message.reply(targetID == event.senderID ? "❌ | Đã xảy ra lỗi không thể làm mới thông tin người dùng của bạn" : `❌ | Đã xảy ra lỗi không thể làm mới thông tin người dùng ${targetID}`);
      }
    } else
      message.SyntaxError();
  }  
